...
class Index extends Component {
					...
					setTimeout(() => {
						...
						if (!this.fininshAnimationIfDone(done)) {
							const interval = setInterval(() => {
								// Check if stuff fetched each 2000ms
	...
	componentWillReceiveProps(nextprops) {
		...
		const x = window.matchMedia("(max-width: 462px)");
		if (!x.matches && showLoading && !loadingScreen) {
	...
	render() {
		...
		const loadingScreen = this.state.loadingScreen;
      ...
      page-wrapper`}
				...
				<NewsletterSignup hideNewsletter={this.props.hideNewsletter} />
			</Fragment>
		);
	}
}
...
const mapStateToProps = state => {
	...
	return {
		...
		notificationActive: state.theme.notificationActive
	};
};
