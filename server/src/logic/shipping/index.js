...
const orderToShipping = order => {
  const { items, orderID, delivery, pacsoftCode } = order;
...
const getOrderShipment = async order => {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      await fs.outputFile(`${publicPath}${basePath}`, labelBase64, {
        encoding: "base64"
      });
...
const getOrderShippingStatus = async (orderID, fetchId = 0) => {
  const data = await getCacheElseFetch(
    domain + "/shipments?fetchId=" + fetchId
  );
  if (!data.shipments) {
